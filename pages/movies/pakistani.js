...
const PakistaniMovies = () => {
  const [Pakistani, setPakistani] = useState([]);
 const router = useRouter()
 const handleCardClick = (title) =>{
  router.push(`/movies/${title}`)
 }
  useEffect(()=>{
   const fetchPakistaniMovies = async()=>{
   const res = await axios.get('http://localhost:5000/movies/pakistani-movies')
   console.log(res.data)
   setPakistani(res.data)
   }
   fetchPakistaniMovies()
  ...
  const totalPages = Math.ceil(Pakistani.length/ITEMS_PER_PAGE)
  ...
  return (
    ...
    <div className=' text-center my-3' style={{
      ...
      borderRadius:'3px'
        ...
        <h3 className='fw-lighter'>Pakistani Movies</h3>
    </div>
    <div className="cards row justify-content-center">
    {Pakistani?.slice(startIndex,endIndex).map((e,index)=>{
    return(
      <>
    ...
    {Array.from({length:totalPages},(_,i)=>i+1).map((e)=>{
      return(
        ...
        <li className='page-item'>
          <button className='page-link' onClick={()=>handlePageChange(e)}>{e}</button>
        </li>
        </>
      )
    })}
...
export default PakistaniMovies